 ...
 * ADTS头中相对有用的信息包括: 采样率, 声道数, 帧长度.
 * 每一个带ADTS头信息的AAC流都会清晰的告诉解码器他需要的这些信息.
 * 一般情况下ADTS的头信息都是7个字节, 分为两部分: 前28bit是 adts_fixed_header,
 * 后28bit是 adts_variable_header.
 */
...
// 设置adts可变的一部分
extern void set_variable_header(
    adts_variable_header *header, const int aac_raw_data_length);

// 解析buff中的adts固定部分头信息
extern void get_fixed_header(
    const unsigned char buff[7], adts_fixed_header *header);

// 解析buff中的adts可变部分头信息
extern void get_variable_header(
    const unsigned char buff[7], adts_variable_header *header);

// 将adts头信息转化为一个64位的整数, PS: 前面8位空着
extern void convert_adts_header2int64(const adts_fixed_header *fixed_header,
    const adts_variable_header *variable_header, uint64_t *header);

// 将adts头信息转化为一个7字节的buff
extern void convert_adts_header2char(const adts_fixed_header *fixed_header,
    const adts_variable_header *variable_header, unsigned char buffer[7]);
