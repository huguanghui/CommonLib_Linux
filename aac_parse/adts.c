...
void convert_adts_header2int64(const adts_fixed_header *fixed_header,
    const adts_variable_header *variable_header, uint64_t *header)
{
    ...
    ret_value <<= 11;
    ret_value
        |= (variable_header->adts_buffer_fullness) & ((int)pow(2, 11) - 1);
    ret_value <<= 2;
    ret_value |= (variable_header->number_of_raw_data_blocks_in_frame)
        & ((int)pow(2, 2) - 1);

    *header = ret_value;
}

void convert_adts_header2char(const adts_fixed_header *fixed_header,
