...
// adt3012 Tx frame design
/*
1,Tx
  TYPE  | data type |
 0x0A00 |   float   | number(uint32) array(range, velocity, power, phase0,
phase1, angleH, angleV) 0x0A01 |   float   | number(uint32) array(range, power,
phase1, angleH) 0x0A02 |   float   | number(uint32) array(range, phase1) 0x0A03
|   float   | breathrate
 ---------------------------------------------------------------------------------------------------
 0x0B00 |   int16   | adc0 I data
 0x0B01 |   int16   | adc0 Q data
 0x0B02 |   int16   | adc1 I data
 0x0B03 |   int16   | adc1 Q data
 0x0B04 |   int16   | adc0 IQ data
 0x0B05 |   int16   | adc1 IQ data
 0x0B06 |   int16   | 0x0055: frame ADC transfer start, 0xaa: frame ADC transfer
stop
 ---------------------------------------------------------------------------------------------------
 0x0Cxx |   int16   | adc0 IQ range fft data(real and image part)
 0x0Dxx |   int16   | adc1 IQ range fft data(real and image part)
 ---------------------------------------------------------------------------------------------------
 0x0Exx |   int16   | adc0 and adc1 IQ velocity fft data
 0x0F00 |   uint16  | 0x0055: taget found; 0x00aa: taget lose
 ---------------------------------------------------------------------------------------------------
 0x0100 |   int16   | printf to gui
...
example
    ...
    store target data : num++;

    msg_tmp.len = 2+num*7;
    multTargetInfo.targetNum = num;
    ...
    {
        TF_SendSimple(ft_tmp,0x0A00,(uint8*)&multTargetInfo,msg_tmp.len);
        TF_DeInit(ft_tmp);
    }
*/
...
typedef struct {
    uint32 targetNum;
    OneTargetInfo multTarget[TARGET_NUM];
} multTargetInfo;
...
typedef struct TF_Msg_ {
     ...
     *
     * This data will be stored in the listener slot and passed to the ID
     * callback via those same fields on the received message.
     */
    void *userdata;
    void *userdata2;
} TF_Msg;
 ...
 */
char TF_Accept(TinyFrame *tf, const uint8_t *buffer, uint32_t count);
 ...
 *accept state {fail =0;sucesse =0xff}
 */
char TF_AcceptChar(TinyFrame *tf, uint8_t c);
