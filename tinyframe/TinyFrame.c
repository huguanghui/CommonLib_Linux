...
static TinyFrame ft_rx;
static TinyFrame ft_tx;

uint8 g_TfRxBuff[TF_RX_BUF_SIZE] = { 0x01, 0x0, 0x0, 0x0, 0x04, 0xFF, 0xFF,
    0xFA, 0x01, 0x01, 0x0, 0x0, 0xFF };
uint32 g_TfWriteCount = 14;
uint32 g_TfReadCount = 0;
...
static const uint16_t crc16_table[256] = { 0x0000, 0xC0C1, 0xC181, 0x0140,
 ...
 * userdata */
static void _TF_FN cleanup_id_listener(
    TinyFrame *tf, TF_COUNT i, struct TF_IdListener_ *lst)
{
    TF_Msg msg;
    if (lst->fn == NULL)
        return;
    ...
    if (lst->userdata != NULL || lst->userdata2 != NULL) {
        ...
        lst->fn(
            tf, &msg); // return value is ignored here - use TF_STAY or TF_CLOSE
    }
...
static inline void _TF_FN cleanup_type_listener(
    TinyFrame *tf, TF_COUNT i, struct TF_TypeListener_ *lst)
{
    lst->fn = NULL; // Discard listener
...
bool _TF_FN TF_AddIdListener(
    TinyFrame *tf, TF_Msg *msg, TF_Listener cb, TF_TICKS timeout)
{
    TF_COUNT i;
...
bool _TF_FN TF_AddTypeListener(
    TinyFrame *tf, TF_TYPE frame_type, TF_Listener cb)
{
    TF_COUNT i;
...
//            msg.userdata = ilst->userdata; // pass userdata pointer to the
//            callback msg.userdata2 = ilst->userdata2; res = ilst->fn(tf,
//            &msg); ilst->userdata = msg.userdata; // put it back (may have
//            changed the pointer or set to NULL) ilst->userdata2 =
//            msg.userdata2; // put it back (may have changed the pointer or set
//            to NULL)

//            if (res != TF_NEXT) {
//                // if it's TF_CLOSE, we assume user already cleaned up
//                userdata if (res == TF_RENEW) {
...
//        }
//    }
//    // clean up for the following listeners that don't use userdata (this
//    avoids data from
//    // an ID listener that returned TF_NEXT from leaking into Type and Generic
//    listeners) msg.userdata = NULL; msg.userdata2 = NULL;
...
//                // TF_RENEW doesn't make sense here because type listeners
//                don't expire = same as TF_STAY
...
//                // TF_RENEW doesn't make sense here because generic listeners
//                don't expire = same as TF_STAY

//                // note: It's not expected that user will have multiple
//                generic listeners, or
//                // ever actually remove them. They're most useful as default
//                callbacks if no other listener
//                // handled the message.
...
char _TF_FN TF_AcceptChar(
    TinyFrame *tf, unsigned char c) // fixed by cxy  20190521
{
    int32 result = 0;
    {
...
// use variables: si - signed int32, b - byte, outbuff - target buffer, pos -
 ...
 */
static inline uint32_t _TF_FN TF_ComposeHead(
    TinyFrame *tf, uint8_t *outbuff, TF_Msg *msg)
{
 ...
 */
static inline uint32_t _TF_FN TF_ComposeBody(
    uint8_t *outbuff, const uint8_t *data, TF_LEN data_len, TF_CKSUM *cksum)
{
    TF_LEN i = 0;
    uint8_t b = 0;
    uint32_t pos = 0;
 ...
 */
static bool _TF_FN TF_SendFrame_Begin(
    TinyFrame *tf, TF_Msg *msg, TF_Listener listener, TF_TICKS timeout)
{
 ...
 * @param length - count
 */
static void _TF_FN TF_SendFrame_Chunk(
    TinyFrame *tf, const uint8_t *buff, uint32_t length)
{
    uint32_t remain;
    uint32_t chunk;
    uint32_t sent = 0;
...
#ifdef GUI
    printf("\n----------------------------------------------\n");
    printf("Print the data of the transmitted TF frame: \n");
#endif // GUI
    ...
    TinyFrame *tf, TF_Msg *msg, TF_Listener listener, TF_TICKS timeout)
{
    ...
    if (msg->len == 0 || msg->data != NULL) {
        ...
        // frame. A multi-part frame is identified by passing NULL to the data
        // field and setting the length. User then needs to call those functions
        // manually
...
bool _TF_FN TF_SendSimple(
    TinyFrame *tf, TF_TYPE type, const uint8_t *data, TF_LEN len)
{
    TF_Msg msg;
    TF_ClearMsg(&msg);
    msg.type = type;
...
bool _TF_FN TF_QuerySimple(TinyFrame *tf, TF_TYPE type, const uint8_t *data,
    TF_LEN len, TF_Listener listener, TF_TICKS timeout)
{
    TF_Msg msg;
    TF_ClearMsg(&msg);
    msg.type = type;
...
bool _TF_FN TF_Query_Multipart(
    TinyFrame *tf, TF_Msg *msg, TF_Listener listener, TF_TICKS timeout)
{
    msg->data = NULL;
...
void _TF_FN TF_Multipart_Payload(
    TinyFrame *tf, const uint8_t *buff, uint32_t length)
{
...
void tinyFrameInit(void)
{
    ...
    g_TfWriteCount = 0;
    g_TfReadCount = 0;
    memset((void *)g_TfRxBuff, 0, sizeof(g_TfRxBuff));
}
/****************************************************************************************
...
TinyFrameRx tinyFramefRx(void)
{
    ...
    for (uint8 i = 0; i < g_TfWriteCount; i++) {
        ret = TF_AcceptChar(&ft_rx, g_TfRxBuff[i]);
        ...
        if (0 == ret) {
            ...
            case 0x0107:
            case 0x0108:
                tempdata = (ft_rx.data[0] + ft_rx.data[1] * 256
                    + ft_rx.data[2] * 256 * 256
                    + ft_rx.data[3] * 256 * 256 * 256);
            ...
            revData.receiveType = ft_rx.type;

            // memset(ft_rx.data, 0, sizeof(ft_rx.data));
            // ft_rx.type = 0x0;
...
void tinyFramefTx(TF_TYPE type, uint8 *data, TF_LEN len)
{
    ...
    TF_SendSimple(&ft_tx, type, data, msg_tmp.len);
}
...
*Description   : uart sends printing information to GUI through tinyframe
protocol *Paramater	   :
*        @ :
*        @ :
*Paramater	selection:
