...
static void CB_EventHandler(struct zw_uptp_message* msg, void* user_data)
{
    ...
    if (MSG_TYPE_RESPONSE == msg->msgType)
    {
        ...
        // 放入缓冲队列
        struct responseNode* node = malloc(sizeof(struct responseNode));
    ...
    while (!info->bTaskDoneExit)
    {
        ...
        else
        {
            ...
            if (node)
            {
                ...
                // 先插入队列再发送
                struct testNode* tn = malloc(sizeof(struct testNode));
            ...
            if (node != NULL)
            {
                ...
                if (task != NULL)
                {
                    ...
                    if (CALL_ASYNC == task->type)
                    {
                        if (task->cb != NULL)
                        {
                            task->cb(task->response.param, task->response.paramLen);
                        }
